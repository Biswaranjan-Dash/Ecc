/*
  ...
  Reads MAX30102 sensor data for heart rate and SpO2
*/
...
void loop() {
  ...
  // Read sensor data from MAX30102
  if (!readSensorData()) {
    // Invalid reading, wait and retry
    delay(3000);
    return;
  }
  
  // Build JSON payload with real sensor data
  String sample = "{\"heart rate\":" + String(heartRate) + ",\"spo2\":" + String(spo2) + "}";
  ...
  // Wait 5 seconds before next reading (sensor needs time between readings)
  delay(5000);
}